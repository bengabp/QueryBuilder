...
from db import db
from response_models import CompletionsResponse, CompaniesSearchResult, SearchRequest, SettingsResponse
...
def get_settings(request: Request, include_companies:bool = Query(alias="includeCompanies", default = True)):
	...
	if include_companies:
		...
		for index, company in enumerate(search_result):
			...
			hq_location = company.get("hq_locations")
			if hq_location:
				address = hq_location.get("address")
			data = {
				"id":index,
